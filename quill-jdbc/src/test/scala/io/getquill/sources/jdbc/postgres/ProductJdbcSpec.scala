...
class ProductJdbcSpec extends ProductSpec {
  ...
  "Product" - {
    ...
    "Single product with inlined Free Variable" in {
      val prd = Product(0L, "test1", 1L)
      val inserted = testPostgresDB.run(quote { product.insert(_.sku -> prd.sku, _.description -> prd.description) })
    ...
    "Single product insert with def" in {
